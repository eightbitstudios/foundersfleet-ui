#!/usr/bin/env python3
"""
Fix AutoPilot positioning on squads.html
"""

import re

def fix_squads_autopilot():
    """Fix the AutoPilot sidebar positioning"""
    ...
    # Pattern to match AutoPilot sidebar and collapsed button
    ...
    # Also remove any standalone AutoPilot sections
    autopilot_pattern2 = r'<div id="autopilot-sidebar".*?</div>\s*(?=<)'
    content = re.sub(autopilot_pattern2, '', content, flags=re.DOTALL)
    
    # Find where to insert AutoPilot - after the main content div closes
    # Look for the closing of the main content area (flex-1 bg-black)
    # The structure should be: left sidebar, main content, then autopilot
    
    # Find the end of main content area - it should be after </main></div>
    main_close_pattern = r'(</main>\s*</div>)'
    ...
    # Replace the pattern - add AutoPilot after main content closes
    replacement = r'\1' + autopilot_html
    content = re.sub(main_close_pattern, replacement, content, count=1)
    
    # Write the fixed content
    with open('squads.html', 'w', encoding='utf-8') as f:
        f.write(content)
    
    print("Fixed AutoPilot positioning in squads.html")

if __name__ == "__main__":
    fix_squads_autopilot()