#!/usr/bin/env python3
"""
Script to add missing AutoPilot to profile.html and squads.html
"""

import re

# The AutoPilot HTML to add (without the Active indicator)
...
def add_autopilot_to_page(filepath):
    """Add AutoPilot to a page before closing body tag"""
    try:
        ...
        # Insert AutoPilot before closing body tag
        content = re.sub(body_pattern, '\n\n' + AUTOPILOT_HTML + '\n\n\\1', content)
        
        # Write back
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
        
        print(f"Added AutoPilot to {filepath}")
        return True
...
def main():
    ...
    for page in pages:
        add_autopilot_to_page(page)
    
    print("\nDone! AutoPilot has been added.")
