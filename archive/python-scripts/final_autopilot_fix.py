#!/usr/bin/env python3
"""
Final script to properly position AutoPilot on Account and Squads pages
"""

import re

# The complete AutoPilot HTML without the Active indicator
AUTOPILOT_HTML = '''
    ...
    <script>
        ...
        // Add the missing function for account tabs
...
def fix_autopilot_final(filepath):
    """Add AutoPilot as the third column in the body flex container"""
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Remove any existing AutoPilot sections first
        content = re.sub(r'<!-- Right Sidebar - AutoPilot -->.*?</script>', '', content, flags=re.DOTALL)
        
        # Find the closing body tag and insert AutoPilot before it
        pattern = r'(</body>)'
        replacement = AUTOPILOT_HTML + '\n\n\\1'
        ...
        print(f"Fixed AutoPilot in {filepath}")
        return True
...
def main():
    ...
    for page in pages:
        fix_autopilot_final(page)
    
    print("\nDone! AutoPilot has been added correctly.")
